...
int get_pattern(char* filename, char **buf, int verbose) {
  ...
  if (verbose > 0)
    printf("Prepare to read pattern from %s\n", filename);
  ...
  if (verbose > 0)
    printf("Read pattern from %s...", filename);
...
int save_page_to_file(int pageid, char* block) {
  ...
  if (!outfile) {
    std::cout << "Could not open output file " << ss.str() << " for write!\n";
    return 1;
  }

  /* write pattern to file */
  fwrite(block, sizeof(char), emu::pc::RAMPAGE_SIZE, outfile);
...
int get_sequential_block(int pageid, char* block, int start) {
  ...
  for (int i = 2; i < 2 + emu::pc::RAMPAGE_SIZE; i++) {
...
int get_page_from_file(int pageid, char* block) {
  /* open input file */
  FILE* infile = 0;
  std::vector<std::string> paths; // give a few possible locations for RAM page files
  paths.push_back("./");
  ...
  if (!infile) {
    std::cout << "Could not find RAM page file.  Tried: " << std::endl;
    for (uint i = 0; i < paths.size(); i++)
  ...
  /* write pattern to file */
  fread(block, sizeof(char), emu::pc::RAMPAGE_SIZE, infile);
...
std::string time_string(bool terse) {
  ...
  if (terse) {
    ...
    sprintf(tmp, "%04d-%02d-%02d_%02d-%02d-%02d", Tm->tm_year + 1900, Tm->tm_mon + 1, Tm->tm_mday, Tm->tm_hour, Tm->tm_min, Tm->tm_sec);
    str = tmp;
  }
  return str;
}
